...
export default function WorldMap({
  ...
  function handleEnter(geo, e) {
    ...
    const speed = c?.abbreviation ? (speedMap[c.abbreviation] ?? null) : null;
    setTooltip({
      name: c?.country ?? geo.properties?.name ?? "—",
      speed,
      x: e.clientX,
      y: e.clientY,
    });
  ...
  const slowest =
    selected.length > 0
      ? Math.max(
          ...selected.map(
            (c) => (fileSizeMB * 8) / Math.max(c.downloadMbps ?? 1, 0.01),
          ),
        )
      : 0;
  ...
  return (
    <div className="map-with-sidebar">
      ...
      >
        {status === "loading" && (
          <div className="map-status-chip status-text muted">
            Loading speed data…
          </div>
        )}
        {status === "error" && (
        ...
        <div className="map-control-cluster">
          <button
            ...
            onClick={() =>
              setPosition((p) => ({
                ...p,
                zoom: clamp(p.zoom + 0.45, MIN_ZOOM, MAX_ZOOM),
              }))
            }
          >
            +
              ...
              {({ geographies }) => (
                <>
                  ...
                  {geographies.map((geo) => {
                    ...
                    const dark =
                      (speedMap[c.abbreviation] ?? 0) > 0 &&
                      Math.sqrt((speedMap[c.abbreviation] ?? 0) / maxSpeed) >
                        0.55;
                    return (
      ...
      <aside className="map-sidebar">
        <div className="eyebrow" style={{ marginBottom: "0.1rem" }}>
          Race Sim
        </div>
            ...
            return c ? (
              <div key={c.abbreviation} className="map-sidebar-racer">
                ...
                <div className="map-sidebar-racer-info">
                  ...
                  <span className="map-sidebar-racer-spd">
                    {c.downloadMbps?.toFixed(0)} Mbps
                  </span>
                </div>
                <button
                  className="map-sidebar-racer-x"
                  onClick={() => removeCountry(c.abbreviation)}
                >
                  ×
                </button>
              </div>
        ...
        <div className="field-stack">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") commitMb(e.target.value);
              }}
