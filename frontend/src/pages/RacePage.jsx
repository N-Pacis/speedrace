...
function Car({ code }) {
  return (
    <svg
      ...
      style={{
        filter:
          "drop-shadow(0 2px 6px rgba(29,78,216,0.35)) drop-shadow(0 1px 3px rgba(0,0,0,0.25))",
        overflow: "visible",
      }}
    >
      <defs>
      ...
      <path
        d="M10,16 L8,24 L10,32 L28,39 L68,41 L82,34 L91,27 L93,24 L91,21 L82,14 L68,7 L28,9 Z"
        fill={CAR_COLOR}
      />
      <path
        d="M28,9 L68,7 L82,14 L68,11 L28,13 Z"
        fill="rgba(255,255,255,0.18)"
      />
      <path
        d="M30,12 L65,10 L73,15 L73,33 L65,38 L30,36 L24,31 L24,17 Z"
        fill={CAR_DARK}
      />
      <image
        href={`https://flagcdn.com/w40/${code.toLowerCase()}.png`}
        x="24"
        y="10"
        width="49"
        height="28"
...
export default function RacePage({ countries = [], mb = 100, onEdit, onBack }) {
  const raceTimes = countries.map(
    (c) => (mb * 8) / Math.max(c.downloadMbps ?? 1, 0.01),
  );
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      countries.forEach((_, i) => {
        ...
        if (carEl) {
          const tw = carEl.parentElement?.offsetWidth ?? 700;
          carEl.style.left = `${pos * Math.max(tw - CAR_W, 0)}px`;
        }
        if (trEl) {
          const tw = trEl.parentElement?.offsetWidth ?? 700;
          trEl.style.width = `${pos * Math.max(tw - CAR_W, 0)}px`;
        }
      ...
      if (finRef.current.length === countries.length) {
      ...
      <div className="race-hdr">
        ...
        {phase !== "countdown" && (
          <div className="race-clock">
            <span className="clock-val" ref={timerRef}>
              0:00.00
            </span>
      ...
      {phase === "countdown" && (
        <div className="countdown-wrap">
          <div className="cd-ring">
            <span className={`cd-num${cdNum === "GO!" ? " cd-go" : ""}`}>
              {cdNum}
            </span>
          </div>
          <p className="cd-label">
            {cdNum === "GO!" ? "GO GO GO!" : "GET READY"}
          </p>
        </div>
      )}
      ...
      <div className="track-outer">
        ...
        {countries.map((c, i) => {
          ...
          return (
            <div
              key={c.abbreviation}
              className={`lane${finished ? " lane-done" : ""}`}
            >
              <div className="lane-lbl">
                <Flag code={c.abbreviation} size={26} />
                <div className="lane-info">
                  <span className="lane-name">{c.country}</span>
                  <span className="lane-spd">
                    {c.downloadMbps?.toFixed(0)} Mbps
                  </span>
                </div>
              </div>
              <div className="lane-road">
                <div
                  className={`road-stripe${isRacing && !finished ? " stripe-moving" : ""}`}
                />
                <div
          ...
          <div className="winner-row">
            ...
            <div className="winner-body">
              ...
              <div className="winner-sub">
                {winner.downloadMbps?.toFixed(1)} Mbps &nbsp;&middot;&nbsp;{" "}
                {raceTimes[finishOrder[0]].toFixed(2)}s
              </div>
            </div>
          </div>
          ...
          <div className="podium">
            {finishOrder.map((ci, rank) => (
              <div key={ci} className="podium-row">
                ...
                <span className="p-spd">
                  {countries[ci].downloadMbps?.toFixed(1)} Mbps
                </span>
