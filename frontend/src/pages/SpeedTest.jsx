...
export default function SpeedTest() {
  ...
  useEffect(() => {
    fetch('https://ipapi.co/json/')
      ...
      .catch(() => { setLocationError('Could not detect location'); setLocationLoading(false) })
  }, [])

  async function runTest() {
    setPing(null); setDownload(null); setUpload(null)
    setLiveSpeed(null); setSubmitStatus(null); setSubmitMessage(null)

    const pings = [], downloads = [], uploads = []
    for (let i = 1; i <= 3; i++) {
      setRun(i)
      setPhase('ping');     pings.push(await measurePing())
      setPhase('download'); setLiveSpeed(0); downloads.push(await measureDownload(setLiveSpeed))
      setPhase('upload');   setLiveSpeed(0); uploads.push(await measureUpload(setLiveSpeed))
    }
    ...
    setPing(finalPing); setDownload(finalDownload); setUpload(finalUpload)
    setLiveSpeed(null); setRun(0); setPhase('done')
    ...
    <div className="page-stack">
      <section className="page-hero">
        ...
        <p className="page-copy">
          Run a quick check on your internet ping, download, and upload speed and help improve the accuracy of your country's average internet speed data
        </p>
      ...
      <section className="surface-card info-panel">
        <div>
          ...
          <div style={{ marginTop: '0.85rem' }}>
            {locationLoading && <p className="status-text muted">Detecting location…</p>}
            {locationError && <p className="status-text error">{locationError}</p>}
            {!locationLoading && !locationError && (
              <>
                <div className="info-value">{ip}</div>
                <p className="info-muted" style={{ marginTop: '3rem' }}>
                  {country ? `${country}${abbreviation ? ` (${abbreviation})` : ''}` : 'Location unavailable'}
                </p>
              </>
            )}
          </div>
        ...
        <div>
          <div className="metric-grid">
            ...
            <div className="metric-item">
              ...
              <div className="metric-value" style={{ marginTop: '0.6rem' }}>
                {download !== null ? download.toFixed(0) : liveSpeed !== null ? liveSpeed.toFixed(0) : '0'}
                <span className="unit">Mbps</span>
              </div>
            </div>
